...
namespace Math
{
	...
	public:
		...
		Vector2<T> NormalizedSafe() const
		{
			const T lengthSq = (x * x) + (y * y);
			return lengthSq == static_cast<T>(0) ? *this : *this / Length();
		}

		Vector2<T>& NormalizeSafe()
		{
			const T lengthSq = (x * x) + (y * y);
	...
	public:
		...
		Vector3<T> NormalizedSafe() const
		{
	...
	public:
		...
		Vector4<T> NormalizedSafe() const
		{
			const T lengthSq = (x * x) + (y * y) + (z * z) + (w * w);
			return lengthSq == static_cast<T>(0) ? *this : *this / Length();
		}

		Vector4<T>& NormalizeSafe()
		{
			const T lengthSq = (x * x) + (y * y) + (z * z) + (w * w);
			if (lengthSq != static_cast<T>(0))
			{
...
// Cross product
	...
	// Normalize (Safe)
	template <typename T>
	inline Vector2<T> NormalizedSafe(const Vector2<T>& vec)
	{
		return vec.NormalizedSafe();
	}

	template <typename T>
	inline Vector3<T> NormalizedSafe(const Vector3<T>& vec)
	{
		return vec.NormalizedSafe();
	}

	template <typename T>
	inline Vector4<T> NormalizedSafe(const Vector4<T>& vec, const Vector4<T>& default = Vector4<T>())
	{
		return vec.NormalizedSafe();
	}

	template <typename T>
	inline Vector2<T>& NormalizeSafe(Vector2<T>& vec)
	{
		return vec.NormalizeSafe();
	}

	template <typename T>
	inline Vector2<T> NormalizeSafe(const Vector2<T>& vec)
	{
		Vector2<T> temp = vec;
		return temp.NormalizeSafe();
	}

	template <typename T>
	inline Vector3<T>& NormalizeSafe(Vector3<T>& vec)
	{
		return vec.NormalizeSafe();
	}
